...
def estadoPedido(codigo):
    ...
    detallesEnvio = payload["detallesEnvio"]
...
def guardarCompra():
    try:
        ...
        payload = {
            "detallesEnvio": json.loads(data["detallesEnvio"]),
            "metodoPago": json.loads(data["metodoPago"]),
            "carrito": json.loads(data["carrito"]),
            "total": float(data["total"])
        }

        response = requests.post(API_URL + "/ticket", json=payload)
        response.raise_for_status()
