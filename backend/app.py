...
def add_ticket():
    try:
        ...
        # validar campos
        keys = ("detallesEnvio", "metodoPago", "carrito", "total")
        for key in keys:
            if key not in data:
        ...
        # verificar si existe
        result = q.ejecutarSQL(q.TICKET_BY_ID, {"id_trackeo": ticket_id})
        if result and result.fetchone():
            return jsonify({"error": "El ticket ya existe"}), 400

        params = {
            "id_trackeo": ticket_id,
            "Total": float(total),
            "Payload": json.dumps({
                "detallesEnvio": data["detallesEnvio"],
...
def get_ticket(id_trackeo):
    try:
        result = q.ejecutarSQL(q.TICKET_BY_ID, {"id_trackeo": id_trackeo})
        ...
        keys = ["ID_TRACKEO", "Total", "Payload", "Estado", "FechaCreacion"]
